...
# Nova for Z^4.
# JM Wed 27 Sep 2017 11:50:16 BST
...
for X in nm.arange ( X_MIN, X_MAX, offset ):
	...
	for Y in nm.arange ( Y_MIN, Y_MAX, offset ):
		...
		while ( abs ( R * (Z**4 - 1) ) >= epsilon and iter_count < maxiter ):
			if ( Z ):
				Z =  Z - ( R * ( Z**4 - 1 ) / ( 4 * Z**3 ) )
...
MsgText = 'Nova for Z^4 = 1 ' + ' and rnum: ' + str( rnum ) + ' and R: ' + str( R )
fname = 'Nova_Z4_R:' + str( R ) + '_' + str( X_MAX ) + str( X_MIN ) + '_Y:' + str( Y_MAX ) + str( Y_MIN ) + '.png'
