...
for X in nm.arange ( X_MIN, X_MAX, offset ):
	...
	for Y in nm.arange ( Y_MIN, Y_MAX, offset ):
		...
		while ( abs ( R * (Z**5 - 1) ) >= epsilon and iter_count < maxiter ):
			if ( Z ):
				Z =  Z - ( R * ( Z**5 - 1 ) / ( 5 * Z**4 ) )
...
MsgText = 'Nova for Z^5 = 1 ' + ' and rnum: ' + str( rnum ) + ' and R: ' + str( R )
fname = 'Nova_Z5_R:' + str( R ) + '_' + str( X_MAX ) + str( X_MIN ) + '_Y:' + str( Y_MAX ) + str( Y_MIN ) + '.png'
